...
//    private final RealMatrix samplingMatrix;
     ...
     * negative.
     */
    public MVN(RandomGenerator rng,
                                          final double[] means,
                                          final double[][] covariances)
            throws SingularMatrixException,
                   DimensionMismatchException,
        ...
        final SingularValueDecomposition sinValDec = new SingularValueDecomposition(covarianceMatrix);
        final EigenDecomposition covMatDec = new EigenDecomposition(covarianceMatrix);

        // Compute and store the inverse.
        covarianceMatrixInverse = sinValDec.getSolver().getInverse();

        // Eigenvalues of the covariance matrix.
        // Get the pseudo-determinant as the product of non-zero eigenvalues
        final double[] covMatEigenvalues = covMatDec.getRealEigenvalues();

        // Compute and store the determinant.
        double det = 1;
        for(double eigenVal : covMatEigenvalues) {
        	if(eigenVal > 1e-12) {
        		det *= eigenVal;
        	}
        }
        covarianceMatrixDeterminant = det;
//
